...
class QForm(forms.Form):
    ...
    export_format = forms.ChoiceField(choices=(('None', 'None'), ('csv', 'csv'), ('json', 'json')), required=True,
                                      widget=forms.Select)
...
class OrganQueryForm(QForm):
    input_type = forms.ChoiceField(choices=(('gene', 'gene'), ('cell', 'cell')), required=True, widget=forms.Select)
    p_value = forms.DecimalField(min_value=0.0, max_value=1.0, required=False)
